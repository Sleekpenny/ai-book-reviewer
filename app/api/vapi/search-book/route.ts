import { searchBookSegmets } from "@/lib/actions/book.actions";
import { NextResponse } from "next/server";

const processBookSearch = async (bookId: unknown, query: unknown) => {
    ...
    const searchResult = await searchBookSegmets(bookIdStr, queryStr, 3);

    if (!searchResult.success || !searchResult.data?.length) {
        return { result: 'No information found about this topic int he book.'}
    }
}
...
export const POST = async (request: Request) => {
    try {
        ...
        console.error('Vapi search-book error:', err);
        return NextResponse.json({
