...
const VAPI_API_KEY = process.env.NEXT_PUBLIC_API_KEY;
...
export const useVapi = (book: IBook) => {
    ...
    const start = useCallback(async() => {
        if (!userId) return setLimitError('Please sign in to access voice a message');
        ...
        try {
            ...
            if (!result.success) {
               setLimitError(result.error || 'Session limit reached. Please upgrade you')
                setStatus('idle');
                return;
            }
